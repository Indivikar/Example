package DragAndDrop.DragFrom3TablesTo1Table.eigenschaften.newSitzungen;

import java.util.List;
import java.util.stream.Collectors;

import DragAndDrop.DragFrom3TablesTo1Table.controller.ControllerNewSitzung;
import DragAndDrop.DragFrom3TablesTo1Table.interfaces.FunktionenAllgemein;
import DragAndDrop.DragFrom3TablesTo1Table.model.TabelleTraktandum;
...
public class ETableViewZuletztErstellt implements FunktionenAllgemein {

	private ControllerNewSitzung controllerNewSitzung;

	private TableView<TabelleTraktandum> tableViewZuletztErstellt;
	private TableColumn<TabelleTraktandum, String> columnSignaturZuletztErstellt;
	private TableColumn<TabelleTraktandum, String> columnTraktandumZuletztErstellt;
	...
	public ETableViewZuletztErstellt(ControllerNewSitzung controllerNewSitzung, TableView<TabelleTraktandum> tableViewZuletztErstellt,
			TableColumn<TabelleTraktandum, String> columnSignaturZuletztErstellt, TableColumn<TabelleTraktandum, String> columnTraktandumZuletztErstellt) {

		this.controllerNewSitzung = controllerNewSitzung;
		this.tableViewZuletztErstellt = tableViewZuletztErstellt;
		this.columnSignaturZuletztErstellt = columnSignaturZuletztErstellt;
		this.columnTraktandumZuletztErstellt = columnTraktandumZuletztErstellt;

		tableViewZuletztErstellt.setPlaceholder(new Label("es konnte kein Traktandum gefunden werden"));
...
//		addContextMenu();
		...
		dragAndDropRowsBetweenTwoTables(controllerNewSitzung, tableViewZuletztErstellt, controllerNewSitzung.getTableViewTraktandenListe(), false, false, true, true);
	...
	public void addColumns(){
		...
		columnSignaturZuletztErstellt.setCellValueFactory(
			    new PropertyValueFactory<TabelleTraktandum, String>("signatur")
			);

		columnSignaturZuletztErstellt.setCellFactory(column -> {
...
//		            	setTooltip(toolTip);
		...
		columnTraktandumZuletztErstellt.setCellFactory(column -> {
...
//		            		toolTipEigenschaften(toolTip); // Zeit verlängern, wo der ToolTip angezeigt wird -> gibt ein Dastellungsfehler, deshalb disabled
		...
		columnTraktandumZuletztErstellt.setCellValueFactory(
...
//		tableViewZuletztErstellt.setRowFactory(
...
//		                    db.setDragView(row.snapshot(null, null));
//		                    ClipboardContent cc = new ClipboardContent();
//		                    cc.put(SystemTrayIcon.SERIALIZED_MIME_TYPE, index);
//		                    db.setContent(cc);
//		                    event.consume();
//		                }
//		            });
...
//	}
	...
	public void filterDaten(){
    	...
    	tableViewZuletztErstellt.setItems(masterListe);
...
//    			!controllerNewTraktandum.getTextArea().getText().equals(savedTraktandum)){
	...
	//		        .sorted()
	    	...
	    	tableViewZuletztErstellt.setItems(gefilterteListe);
